...
Iris_path = \
  ...
  strip().decode('utf-8')
ESP_path = os.path.join(Iris_path,"../esp")
hls4ml_model_path = os.path.join(Iris_path,"model/hls4ml_project")
run_script_template_path = os.path.join(Iris_path,"scripts/run_accgen_template.sh")
...
espacc_path = os.path.join(ESP_path,"accelerators/hls4ml/iris_hls4ml/hw/src/espacc.cc")
line_modifications = [
  ["unsigned short size_in1, size_out1;", "//"],
  ["myproject(_inbuff, _outbuff, size_in1, size_out1);", "myproject(_inbuff, _outbuff);"]]
  ...
  for i in range(0, len(lines)):
    for j in range(0, len(line_modifications)):
      if line_modifications[j][0] in lines[i]:
        lines[i] = lines[i].replace(line_modifications[j][0],line_modifications[j][1])
